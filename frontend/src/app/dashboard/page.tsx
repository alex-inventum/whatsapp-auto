...
export default function Dashboard() {
  ...
  const cards = [
    { label: 'Mensajes Hoy', value: stats.messages, color: 'var(--primary)', glow: 'rgba(0,168,132,0.2)', bg: 'rgba(0,168,132,0.08)', icon: 'M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z' },
    { label: 'Conversaciones', value: stats.conversations, color: '#7B5EA7', glow: 'rgba(123,94,167,0.2)', bg: 'rgba(123,94,167,0.1)', icon: 'M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z' },
    { label: 'Reglas Activas', value: stats.rules, color: '#38BDF8', glow: 'rgba(56,189,248,0.2)', bg: 'rgba(56,189,248,0.08)', icon: 'M12 2L4.5 20.3l.7.7L12 18l6.8 3 .7-.7z' },
    { label: 'Pago Pendiente', value: stats.pending, color: '#FB923C', glow: 'rgba(251,146,60,0.2)', bg: 'rgba(251,146,60,0.08)', icon: 'M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z' },
  ];
  ...
  return (
    <div>
      ...
      <div className="p-6 md:p-8 space-y-6">
          ...
          {cards.map((c, i) => (
            <div key={c.label} className="card p-5 animate-fade-in" style={{ animationDelay: `${i * 0.08}s` }}>
              <div className="flex items-center justify-between">
                ...
                <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: c.bg, boxShadow: `0 0 16px ${c.glow}` }}>
                  <svg width="16" height="16" fill={c.color} viewBox="0 0 24 24"><path d={c.icon} /></svg>
                </div>
              </div>
              <p className="text-3xl font-bold mt-3 tracking-tight" style={{ color: 'var(--text-primary)' }}>{c.value}</p>
            </div>
          ))}
        </div>
        ...
        {/* Quick actions */}
        <div>
          <h3 className="font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Acciones Rapidas</h3>
